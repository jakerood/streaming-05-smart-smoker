"""
    This program listens for food B temperature messages continuously and checks for stalls.
...
def callback(ch, method, properties, body):
    ...
    if len(foodB_temps) == food_window:
        initial_temp = foodB_temps[0]
        current_temp = foodB_temps[-1]
        ...
        if abs(temp_change) <= food_stall_threshold:
            logger.warning(f"{timestamp}: FOOD B STALL ALERT! Temperature change is {temp_change} F in {food_window / 2} minutes.")
...
def main(hn: str = "localhost", qn: str = foodB_queue):
    """ Continuously listen for food B temperature messages on a named queue."""
    ...
    try:
        ...
        logger.info(" [*] Waiting for food B temperature messages. To exit press CTRL+C")
